...
export class Block {
  ...
  async lookup(blockId, lt, utime) {
    ...
    try {
      ...
      if (blockHeader.info.shard.workchain_id !== blockId.workchain) throw Error("Invalid workchain");
      if (blockHeader.info.not_master != (blockId.workchain !== -1)) throw Error("Invalid not_master");
      if (utime && blockHeader.info.gen_utime !== utime) throw Error("Invalid utime");
      if (lt && (lt.lt(blockHeader.info.start_lt) || lt.gt(blockHeader.info.end_lt))) throw Error("Invalid lt");
      ...
      for (let i = 0; i < 10000; i++) {
        ...
        for (let k of blockProof.steps) {
          ...
          if (k.from.seqno > 0) {
            ...
            if (
              proofCell[0].type != 3 ||
              !compareBytes(proofCell[0].refs[0].getHash(0), new Uint8Array(k.from.root_hash.buffer))
            )
          ...
          if (k.to.seqno > 0) {
            ...
            if (Boolean(blockTo.info.key_block) !== k.to_key_block) throw Error("Incorrect is_key_block value");
          ...
          if (!fwd) {
            ...
            if (stateProof.custom.prev_blocks._ !== "HashmapAugE") throw Error("no prev blocks found");
            if (!stateProof.custom.prev_blocks.map.has(k.to.seqno.toString(16))) throw Error("no prev blocks found");
            ...
            for (let j = 0; j < k.signatures.signatures.length; j++) {
              ...
              if (node_list[s] === undefined) throw Error("signature set contains unknown NodeIdShort");
  ...
  async getShards(blockId) {
    ...
    try {
      ...
      if (!compareBytes(shardHashesHash, mcShardState.custom.shard_hashes.hash)) throw Error("Invalid shard info hash");
  ...
  async getAccountState(blockId, accountAddr) {
    ...
    try {
      ...
      if (address.wc !== -1) {
        ...
        if (shardBlockHeader.info.shard.workchain_id !== blockId.workchain) throw Error("Invalid workchain");
        if (shardBlockHeader.info.not_master != (blockId.workchain !== -1)) throw Error("Invalid not_master");
      ...
      if (blockHeader.info.shard.workchain_id !== shardBlockId.workchain) throw Error("Invalid workchain");
      if (blockHeader.info.not_master != (shardBlockId.workchain !== -1)) throw Error("Invalid not_master");
      ...
      const accountHash =
        accountValue.account._ === "Cell" ? accountValue.account.getHash(0) : accountValue.account.cell.getHash(0);

      // get & check account hash
      if (accountCell.type !== Cell.OrdinaryCell || !compareBytes(accountCell.getHash(0), accountHash))
