...
import { InstantiateMsg as Cw20InstantiateMsg, MinterResponse } from '@oraichain/oraidex-contracts-sdk/build/OraiswapToken.types';
...
describe('Real Ton data tests', () => {
  ...
  beforeAll(async function () {
    ...
    const { rawBlockData, initialKeyBlockInformation } = await queryKeyBlock(liteClient, liteEngine, masterchainInfo.last.seqno);
    ...
    // deploy contracts
    const validatorDeployResult = await deployContract(client, sender, { boc: initialKeyBlockBoc }, 'bridge-validator', 'cw-tonbridge-validator');
  ...
  it('Verify updated validator signatures in new block real data', async () => {
    ...
    const { parsedBlock, rawBlockData, initialKeyBlockInformation: blockId } = await queryKeyBlock(liteClient, liteEngine, blockToCheck);
  ...
  it('shard block test real data', async () => {
    ...
    expect(
      await validator.isVerifiedBlock({
        rootHash: '5d5215c4dd5e2dc3e8b0640339303135cd7296c577e37d1f0e1781cde6fb9629'
      })
    ).toEqual(true);
    expect(
      await validator.isVerifiedBlock({
  ...
  it("bridge contract reads real data from masterchain block's transaction", async () => {
    ...
    const result = await fetch(`https://toncenter.com/api/v3/transactions?account=${addressRaw}&limit=100&offset=0&sort=desc`).then((data) => data.json());
    const { block_ref, mc_block_seqno, hash, lt } = result.transactions.find((tx) => {
      return Buffer.from(tx.hash, 'base64').toString('hex') === '25d1ed22d37fa5ec44b4426f00f33ee3f59e527e8252b9da266172d342c0f5fd';
    });
