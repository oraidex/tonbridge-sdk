...
import { TonbridgeBridgeClient, TonbridgeBridgeTypes, TonbridgeValidatorClient, TonbridgeValidatorTypes } from '@oraichain/tonbridge-contracts-sdk';
import { Cell } from '@ton/core';
import { queryAllValidatorCandidates, queryAllValidators } from './common';
import { data, findBoc, initialValidatorsBlockRootHash, initialValidatorsList, updateValidators, updateValidatorsRootHash } from './data/transaction-1';
...
describe('Tree of Cells parser tests 1', () => {
  ...
  beforeAll(async function () {
    // deploy contracts
    const validatorDeployResult = await deployContract(client, sender, { boc: findBoc('set-validators').toString('hex') } as TonbridgeValidatorTypes.InstantiateMsg, 'bridge-validator', 'cw-tonbridge-validator');
  ...
  it('Verify updated validator signatures in new block', async () => {
    ...
    validators.forEach((validator) => {
      const item = updateValidators.find((v) => v.node_id.toLocaleLowerCase() === validator.node_id.toLocaleLowerCase());
      expect(item).not.toBeUndefined();
      expect(validator.pubkey).toEqual(item?.pubkey);
    });
