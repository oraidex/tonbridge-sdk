...
(async () => {
  ...
  // Create Client
  const shardInfo = await client.lookupBlockByID({
    seqno: 43884169,
    shard: "2000000000000000",
    workchain: 0,
  });
  const shardProof = await engine.query(
  ...
  for (let i = 0; i < shardProof.links.length; i++) {
    ...
    if (i > 0) {
      ...
      console.log(
        "prev block root hash: ",
        Buffer.from(block.info.prev_ref.prev.root_hash).toString("hex")
      );
