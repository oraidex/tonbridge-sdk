...
(async () => {
  ...
  assert(
    Buffer.from(stateProofCellHash).toString("hex") ===
      initialKeyBlockInformation.rootHash.toString("hex")
  );
  ...
  assert(
    Buffer.from(newStateHash).toString("hex") ===
      Buffer.from(configProofHash).toString("hex")
  );

  const configProofCellTonRocks = await TonRocks.types.Cell.fromBoc(
    config.configProof
  );
  const configStateData = loadShardStateUnsplit(
    configProofCellTonRocks[0].refs[0]
  );
