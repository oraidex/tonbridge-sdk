...
import {
  loadBlockExtra,
  loadTransaction,
} from "@oraichain/tonbridge-utils/build/blockchain/BlockParser";
...
(async () => {
  ...
  const mcBlockId = fullBlock.shards.find(
    (shard) => shard.seqno === mc_block_seqno
  );
  const wantedShardInfo = fullBlock.shards.find(
    (shard) => shard.seqno === block_ref.seqno
  );

  // PROVE that our shard block is valid via shard proofs.
  const blockProof = await engine.query(
  ...
  for (let i = 0; i < blockProof.links.length; i++) {
    ...
    const blockRootHash = Buffer.from(
    ...
    if (i === 0) {
      console.log(
        "block root hash i === 0: ",
        blockRootHash,
        mcBlockId.rootHash.toString("hex")
      );
      ...
      const shardDescrs: any[] = Block._shardGetFromHashmap(
        block.extra.custom.shard_hashes,
        wantedShardInfo.workchain
      );
      shardDescrs.forEach((shardDescr) =>
        validatedShardBlockHashes.push(
          Buffer.from(shardDescr.root_hash).toString("hex")
        )
      );
    }
    if (i > 0) {
      ...
      const validatedBlockHash = Buffer.from(
        block.info.prev_ref.prev.root_hash
      ).toString("hex");
      validatedShardBlockHashes.push(validatedBlockHash);
    }
    if (
      validatedShardBlockHashes.length > 0 &&
      validatedShardBlockHashes.some(
        (hash) => hash === wantedShardInfo.rootHash.toString("hex")
      )
    ) {
  ...
  const transactionProofCell = await TonRocks.types.Cell.fromBoc(
    transaction.proof
  );
  const transactionDetailCell = await TonRocks.types.Cell.fromBoc(
    transaction.transaction
  );
  const transactionDetail = loadTransaction(transactionDetailCell[0], {
    cs: 0,
    ref: 0,
  });
  // console.log("transaction detail: ", transactionDetail);
  const merkleProofHash = Buffer.from(
    transactionProofCell[0].refs[0].getHash(0)
  ).toString("hex");
  ...
  const blockExtra = loadBlockExtra(transactionProofCell[0].refs[0].refs[3], {
    cs: 0,
    ref: 0,
  });
  ...
  blockExtraMap.forEach(async (value, key) => {
    const txHash = Buffer.from(
      value.value.transactions.map.get(Number(lt).toString(16)).value.getHash(0)
    ).toString("hex");
